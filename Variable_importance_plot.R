...
varimp_df_median <- as.data.frame(VARIMP) 
names(varimp_df_median) <- paste0("Fold", 1:ncol(VARIMP))
varimp_df_median$Variable <- rownames(VARIMP)
varimp_df_median$MedianImportance <- apply(varimp_df_median, 1, median, na.rm = TRUE)
varimp_df_median$MedianImportance <- as.numeric(varimp_df_median$MedianImportance)
varimp_df_median <- varimp_df_median[order(-varimp_df_median$MedianImportance), ]
